  ...
  <div class="body">
    ...
    <div class="list">
      <table>
        ...
        <th>${message(code: 'study.name.label', default: 'Study')}</th>
        <th>${message(code: 'instrument.name.label', default: 'Instrument')}</th>
        <th>${message(code: 'format.name.label', default: 'Format')}</th>

        <g:sortableColumn property="name" title="${message(code: 'batchCreationConfig.name.label', default: 'Selection Name')}" />
        ...
        <g:each in="${batchCreationConfigInstanceList}" status="i" var="batchCreationConfigInstance">
          <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
            ...
            <td><g:link action="edit" id="${batchCreationConfigInstance.id}">
