...
<fieldset class="maroonBorder"><legend
	style="margin-left: 0.5em;">Batches mailed on <g:formatDate
	format="yyyy-MM-dd" date="${referenceDate}" /></legend>
...
<ul>
	...
	<g:each var="b" in="${sentBatchInstanceList}">
		...
		${b.id} - ${b.primaryInstrument.study} ${b.primaryInstrument} generated on <g:formatDate
			format="yyyy-MM-dd" date="${b.dateCreated}" /></li>
	</g:each>
</ul>
</fieldset>
...
<fieldset class="maroonBorder"><legend
	style="margin-left: 0.5em;">Batches generated, but not mailed
2 weeks prior to <g:formatDate format="yyyy-MM-dd"
	date="${referenceDate}" /></legend>
