...
<div id="${'contactInfoBox' + type}" class="contactInfoBox">
    ...
    <%--<div><strong>${personInstance}</strong></div>--%>
		<table>
			...
			<tbody>
				...
				<g:if test="${type == 'address'}">
					...
					<g:each in="${personInstance.streetAddresses.sort { a,b -> b.dateCreated <=> a.dateCreated ?: a.preferredOrder <=> b.preferredOrder } }" var="contactInfoInstance" status="count" >
						<tr class="${ count.mod(2) == 0 ? 'odd': 'even' }">
							...
							<td><g:if test="${contactInfoInstance.active}"><g:if test="${contactInfoInstance.preferredOrder == 0}">*</g:if></g:if></td>
							<td>
							...
							<td>
								<input size="12" id="${'datepicker_' + contactInfoInstance.id}" name="${'endDate_' + contactInfoInstance.id}" type="text" title="Entering an End Date will make this entry inactive" value="<g:formatDate date='${contactInfoInstance.endDate}' format='MM/dd/yyyy' />" />
							</td>
						</tr>
					</g:each>
				</g:if>
        ...
        <div>* Preferred</div>
        <span class="buttons">
            <g:submitToRemote url="[ controller:'person', action:'updateContactInfo' ]" class="save" value="Save" update="${'contactInfoBox' + type}" onSuccess="updateMessage()" onComplete="showElementById('${buttonId}')" />
            <g:submitToRemote url="[ controller:'person', action:'cancelContactUpdate' ]" class="cancel" value="Done" update="${'contactInfoBox' + type}" onComplete="showElementById('${buttonId}')" />
        </span>
    </g:form>
</div>
