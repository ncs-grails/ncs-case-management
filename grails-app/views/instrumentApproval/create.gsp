...
<g:set var="entityName"
	value="${message(code: 'instrumentApproval.label', default: 'Instrument Approval')}" />
...
<span class="menuButton"><g:link class="list"
	controller="instrumentHistory" action="show"
	id="${instrumentApprovalInstance?.instrumentHistory?.id}">Back to Instrument Revision</g:link></span>
</div>
<div class="body">
...
<g:if test="${flash.message}">
	...
	<div class="prop"><g:hiddenField name="instrumentHistory.id"
		value="${instrumentApprovalInstance?.instrumentHistory?.id}" />
	<h2>
	${instrumentApprovalInstance?.instrumentHistory?.isInitial.toString().capitalize()}
	${instrumentApprovalInstance?.instrumentHistory?.instrument}, v${instrumentApprovalInstance?.instrumentHistory?.itemVersion}
	</h2>
	</div>
	...
	<g:textArea name="comments"
		value="${instrumentApprovalInstance?.comments}" rows="5" cols="40" />
	</span></div>
