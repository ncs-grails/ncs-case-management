...
<html>
    ...
    <body>
        <div class="nav">
            ...
            <span class="menuButton"><g:link class="edit" controller="appointment" action="edit" id="${appointmentInstance?.id}">
            	Back To Appointment</g:link></span>
        </div>
        <div class="body">
            <h1>Incentive for ${appointmentInstance?.type}</h1>
            <h2>
            	<g:formatDate date="${appointmentInstance?.startTime}" format="MM/dd/yyyy hh:mm a" />, 
            	${appointmentInstance?.person}
            	${appointmentInstance?.dwellingUnit}
           	</h2>
            ...
            <g:form action="saveAppointmentIncentive" >
