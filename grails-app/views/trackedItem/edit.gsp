		...
		<h1><g:message code="default.edit.label" args="[entityName]" /> ID: ${trackedItemInstance?.id}</h1>
	       ...
	       <div class="prop">
	           ...
	           <span class="value ${hasErrors(bean: trackedItemInstance, field: 'streetAddress', 'errors')}">
	               <%-- <g:select name="streetAddress.id" from="${edu.umn.ncs.StreetAddress.list()}" optionKey="id" value="${trackedItemInstance?.streetAddress?.id}" noSelection="['null': '']" />  --%>
	       ...
	       <div class="prop">
	           ...
	           <span class="value ${hasErrors(bean: trackedItemInstance, field: 'result', 'errors')}">
	               <g:datePicker name="receiptDate" value="${trackedItemInstance?.result?.receivedDate}" precision="day" />
	           </span>	       
